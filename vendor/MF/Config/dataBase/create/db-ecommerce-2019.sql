...
CREATE TABLE `db_ecommerce`.`tb_products` (
  ...
  `desparcelas` INT NOT NULL DEFAULT 1,
  `dtregister` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE tb_coments(
    ...
    CONSTRAINT fk_coments_users_idx FOREIGN KEY (iduser) REFERENCES tb_users (iduser),
    CONSTRAINT fk_coments_product_idx FOREIGN KEY (idproduct) REFERENCES tb_products (idproduct)    
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `tb_addresses` (
  ...
  `idperson` int(11) NOT NULL,
  `desaddress` varchar(128) NOT NULL,
  `descomplement` varchar(200) DEFAULT NULL,
  `descity` varchar(200) NOT NULL,
  `desstate` varchar(200) NOT NULL,
  `desnumber` VARCHAR(15) NULL,
  `descountry` varchar(200) NOT NULL,
  `deszipcode` char(10) NOT NULL,
  `desdistrict` varchar(200) NOT NULL,
  `dtregister` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`idaddress`),
  KEY `fk_addresses_persons_idx` (`idperson`),
  CONSTRAINT `fk_addresses_persons` FOREIGN KEY (`idperson`) REFERENCES `tb_persons` (`idperson`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;
DROP TABLE IF EXISTS tb_orderspayment;

CREATE TABLE tb_orderspayment (
	  idpayment INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    despayment VARCHAR(60) NOT NULL,
    dtregister TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `tb_orders` (
  ...
  `idpayment` int(11) NOT NULL DEFAULT 1,
